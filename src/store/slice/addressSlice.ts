  ...
  reducers: {
    addItem(state, action: PayloadAction<Address>) {
      ...
      if (action.payload.default === true) {
        ...
        state.list.map(item =>
          item.id === state.default ? {...item, default: false} : item,
        );
        state.default = id;
      }
    },
    deleteItem(state, id: PayloadAction<number>) {
      ...
      return {
        list: state.list.filter(item => item.id !== id.payload),
        default: id.payload === state.default ? null : state.default,
      };
    },
    ...
    changeDefault(
      state,
      action: PayloadAction<{isDefault: boolean; id: number}>,
    ) {
    ...
    initList(
      state,
      action: PayloadAction<{list: AddressStore[]; default: number | null}>,
    ) {
      state.list = action.payload.list;
      state.default = action.payload.default;
    },
    appendList(state, action: PayloadAction<AddressStore[]>) {
